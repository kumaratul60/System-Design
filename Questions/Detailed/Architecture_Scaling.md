...
A hybrid of SSG and SSR. It allows you to update static pages *after* the build, on a per-page basis, without rebuilding the entire site.
...
1.  **Module Federation (Webpack 5):** The modern standard. It allows a "Host" application to dynamically load code from "Remote" applications at runtime.
    *   *Pros:* Independent deployments, decoupled build pipelines, and shared dependencies (to avoid downloading React multiple times).
...
**Question:** Why is Accessibility not optional and how do you ensure it?

**Answer:**
Accessibility ensures that everyone (including people with disabilities) can use your app. It is also important for SEO and legal compliance.

**Key Steps:**
1.  **Semantic HTML:** Use `<button>`, `<nav>`, `<main>` instead of `<div>` for everything.
2.  **ARIA Labels:** Use `aria-label`, `aria-describedby` when semantic HTML isn't enough.
3.  **Keyboard Navigation:** Ensure all interactive elements are focusable (`tabindex`) and have visible focus states.
4.  **Contrast Ratios:** Ensure text is readable against the background (WCAG AA/AAA standards).
5.  **Automated Testing:** Use `axe-core` or `eslint-plugin-jsx-a11y`.
