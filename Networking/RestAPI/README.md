...
> **Answer:** In distributed systems, network timeouts are common. A client might send a POST request, the server processes it, but the ACK is lost. The client retries, leading to a duplicate (e.g., double charging a credit card).
>
> - **The Solution:** Use **Idempotency Keys** (usually a UUID generated by the client). The server stores the key in a database (like Redis) for a short window. If a second request with the same key arrives, the server returns the _cached_ response instead of re-processing.

### Q2: Is "Versioning" a sign of failure in API design? Discuss Versioning vs. Evolution.

> **Answer:** Deep architectural take: Every time you increment `/v2`, you force your entire ecosystem to migrate, creating technical debt.
>
> - **API Evolution:** Prefer **Additive Changes** (adding new fields) and **Graceful Deprecation** (using headers like `Deprecation: true`).
> - **When to Version:** Only when a fundamental change in the **Business Logic** or **Security Model** occurs that cannot be mapped back to the old schema.

### Q3: Why is HATEOAS rarely used in modern production REST APIs?

> **Answer:** While HATEOAS (hypermedia links) is a core REST constraint, it adds significant payload overhead and complexity for clients.
>
> - **The Reality:** Most clients (Frontends) are built with hardcoded logic tied to a specific API version. The "self-discovering" nature of HATEOAS is often redundant in a world of strict OpenAPI/Swagger documentation.
> - **Where it Shines:** In highly dynamic workflows where the _available actions_ change frequently based on state (e.g., a banking transfer that can be 'cancelled' or 'reversed' depending on its status).

### Q4: PATCH vs. PUT: Which one is safer and why?

> **Answer:** **PUT** is idempotent and safer for full replacements. If the network fails, you can retry PUT 100 times and the result is the same.
>
> - **PATCH** is more efficient (partial update) but trickier. For example, a `PATCH` that "appends to an array" is **NOT** idempotent. Retrying it will result in multiple appends. To make PATCH safe, you must use **JSON Merge Patch** or **JSON Patch (RFC 6902)**.
